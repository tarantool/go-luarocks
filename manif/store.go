...
// legacyRockspecKey is the fixed rock_manifest key upstream used before it
// switched to the versioned "<name>-<version>.rockspec" filename; still
// accepted as a fallback when the filename was not recorded.
const legacyRockspecKey = "rockspec"
...
//
// The on-disk rock_manifest is assignments-mode: a single
// `rock_manifest = { ... }` key emitted by writer.lua via
// persist.save_from_table, so ReadRock parses it directly and extracts
// the rock_manifest key. Module-mode `return { ... }` is not handled.
...
func manifestToTable(m *rocks.Manifest) map[string]any {
	...
	for pkg, versions := range m.Repository {
		...
		for ver, entry := range versions {
			...
			// Upstream store_results always sets entrytable.modules and
			// entrytable.commands for an installed arch entry, rendering {}
			// when empty (writer.lua:217-227). Emitting them unconditionally
			// keeps the on-disk manifest byte-for-byte with the reference even
			// for doc-only or command-less rocks.
			if entry.Arch == "installed" {
	...
	// Upstream update_dependencies runs unconditionally, so a manifest with any
	// installed rock always carries a top-level dependencies table (emitted as
	// {} when empty). Always emit it for parity with make_manifest.
	{
		deps := map[string]any{}
				...
				for i, d := range list {
					...
					if len(d.Constraints) > 0 {
						...
						for j, c := range d.Constraints {
							cs[j] = map[string]any{
								"op":      c.Op,
								"version": versionToTable(c.Version),
							}
						}
...
func extractRepoEntry(v any) (rocks.RepoEntry, error) {
	...
	if deps, ok := entry["dependencies"].(map[string]any); ok && len(deps) > 0 {
		re.Dependencies = map[string]string{}

		for k, v := range deps {
			s, ok := v.(string)
			if !ok {
				return rocks.RepoEntry{}, fmt.Errorf("dependencies.%s is %T, want string", k, v)
			}
		...
		if cs, ok := m["constraints"].([]any); ok {
			for _, c := range cs {
				...
				d.Constraints = append(d.Constraints, rocks.VersionConstraint{
					Op:      op,
					Version: constraintVersionFromValue(cm["version"]),
				})
			}
		}
...
// constraintVersionFromValue reconstructs a rocks.Version from a serialized
// constraint version, accepting either the parsed-version table form
// ({ 3, 1, 0, string = "3.1.0" }) or a bare string (older/hand-written
// manifests). It is the read-side inverse of versionToTable.
func constraintVersionFromValue(v any) rocks.Version {
	switch x := v.(type) {
	case string:
		return rocks.Version{Raw: x}
	case *table:
		out := rocks.Version{}
		if s, ok := x.str["string"].(string); ok {
			out.Raw = s
		}

		for _, k := range x.numKeys {
			if f, ok := numToFloat(x.num[k]); ok {
				out.Components = append(out.Components, f)
			}
		}

		if rev, ok := numToFloat(x.str["revision"]); ok {
		...
		return out
	case map[string]any:
		// A version table with no numeric components (empty Components) parses
		// back as a pure string-keyed map rather than the mixed *table form.
		out := rocks.Version{}
		if s, ok := x["string"].(string); ok {
			out.Raw = s
		}

		if rev, ok := numToFloat(x["revision"]); ok {
			out.Revision = int(rev)
			out.HasRevision = true
		}
		...
		return out
	default:
		return rocks.Version{}
	}
}

// numToFloat coerces a parser-produced numeric value (int64 or float64) to
// float64.
...
// versionToTable serializes a parsed version as the Lua table upstream stores
// for a dependency constraint (vers.parse_version): the numeric components in
// the array part plus a "string" field, e.g. { 3, 1, 0, string = "3.1.0" }
// (glr-4x1). A revision key is emitted only when the version carried an
// explicit "-N" suffix (vers.lua:110-114).
func versionToTable(v rocks.Version) map[any]any {
	out := map[any]any{"string": v.Raw}
...
func rockManifestToTable(rm *rocks.RockManifest) map[string]any {
	out := map[string]any{}
	// Upstream keys the rockspec entry by its versioned filename
	// (<name>-<version>.rockspec), not a fixed "rockspec" key (writer.lua walks
	// the install dir). Fall back to the legacy "rockspec" key only if the
	// filename was not recorded.
	if rm.Rockspec != "" {
		key := rm.RockspecFile
		if key == "" {
			key = legacyRockspecKey
		}
...
func rockManifestFromTable(t map[string]any) (*rocks.RockManifest, error) {
	...
	// The rockspec entry is keyed by its versioned filename (a top-level string
	// value whose key ends in ".rockspec"), or the legacy fixed "rockspec" key.
	for key, v := range t {
		if key != legacyRockspecKey && !strings.HasSuffix(key, ".rockspec") {
			continue
		}

		s, ok := v.(string)
		if !ok {
			return nil, fmt.Errorf("rock_manifest.%s is %T, want string", key, v)
		}

		if key != legacyRockspecKey {
			rm.RockspecFile = key
		}
